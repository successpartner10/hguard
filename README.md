...
├── worker/                   # CLOUDFLARE WORKERS backend (Phase 6) — LIVE at ai-home-guard.sandipy7979.workers.dev
...
|---|---|---|
| 2026-08-10 | **Cloudflare backend LIVE in production.** Deployed `worker/` to `https://ai-home-guard.sandipy7979.workers.dev` via API token; validated in production with the full suite (28/28 protocol + 24/24 browser E2E against the live Worker, incl. real clip uploads, QR pairing, live WebRTC, DO persistence). Clip/thumb store moved from R2 (requires activation/card) to the Registry DO's SQLite storage — pure Workers free tier. QR codes render as SVG via vendored `qrcode` core+svg-tag bundle (package browser-field remapping broke deep imports). Test runner supports `https`/`wss` to target the live Worker. | `worker/src/{index,registry}.ts`, `worker/src/vendor/qr.js`, `worker/wrangler.toml`, `test/integration.test.js`, `README.md` |
